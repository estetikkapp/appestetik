...
async function loadAgenda(date: string) {
  ...
  // Rango del día en zona AR: 00:00 AR → 23:59:59 AR
  ...
  const appointments: AppointmentWithRelations[] = (apptsResult.data ?? []).map((a) => {
    ...
    return {
      ...
      status: a.status,
      notes: a.notes,
      client: clientRel ? { id: clientRel.id, full_name: clientRel.full_name, phone_e164: clientRel.phone_e164 } : null,
      service: serviceRel
        ? {
            id: serviceRel.id,
        ...
        : null,
      professional_name: a.professional_id ? professionalsById.get(a.professional_id) ?? null : null,
...
export default async function AgendaPage({
  ...
  const data = await loadAgenda(date);
    ...
    <div className="mx-auto max-w-6xl space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-stone-500">Turnos del día con vista hora a hora.</p>
        </div>
