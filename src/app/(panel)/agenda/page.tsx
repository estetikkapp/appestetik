...
export default async function AgendaPage({
  ...
  const data =
    view === 'week' ? await loadWeekAgenda(weekStart) : await loadDayAgenda(date);

  if (!data) {
    return <div className="text-sm text-stone-500">No hay organización activa.</div>;
  }

  // Default date for "create appointment" sheet: el día pedido (o hoy en vista semana)
  const defaultDateForSheet = view === 'week' ? todayInAr() : date;
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-stone-500">
            {view === 'day' ? 'Turnos del día con vista hora a hora.' : 'Vista semanal compacta.'}
          </p>
        </div>
